...
/**
 * ka系统合同信息(KaSystemContractmsg)表控制层
 *
 * @author makejava
 * @since 2020-10-30 15:14:18
 */
@Api(value = "合同信息")
@Controller
@RequestMapping("kaSystemContractmsg")
public class KaSystemContractmsgController extends BaseController {
    /**
     * 服务对象
     */
    @Resource
    private KaSystemContractmsgService kaSystemContractmsgService;
     ...
     * @return
     */
    @Log(title = "合同信息导入", businessType = BusinessType.IMPORT)
    @RequiresPermissions("kaSystemContractmsg:insert")
    @PostMapping("importExcel")
    @ResponseBody
    @Transactional(rollbackFor = Exception.class)
    @ApiOperation(value = "导入合同信息",notes = "根据导入的文件导入数据到合同信息表",response = JSONResult.class,httpMethod = "POST")
    public JSONResult importExcel(@ApiParam(name = "file",value = "导入合同信息文件file",required = true) @RequestParam("file") MultipartFile file, HttpServletResponse response){
        boolean boo = false;
        //设置表名
        String tablename = "ka_system_contractmsg";
        ...
        try {
            ...
            for (int i=1;i<sheetAt.getPhysicalNumberOfRows();i++){
                ...
                int msg;
                if (this.kaSystemContractmsgService.selectmsg((String) bean.beanMap.get("contract_code"))>=1){
                    msg = this.kaSystemContractmsgService.updatemsg(bean.beanMap);
                }else {
                    //调用添加数据的方法
                    msg = this.kaSystemContractmsgService.insertmsg(bean.beanMap);
                }
                if (msg>0){
                    boo = true;
                }
