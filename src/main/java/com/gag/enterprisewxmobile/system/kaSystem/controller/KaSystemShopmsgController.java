...
/**
 * ka系统门店信息(KaSystemShopmsg)表控制层
 *
 * @author makejava
 * @since 2020-10-30 15:14:22
 */
@Api(value = "门店信息")
@Controller
@RequestMapping("kaSystemShopmsg")
public class KaSystemShopmsgController extends BaseController {
    /**
     * 服务对象
     */
    @Resource
    private KaSystemShopmsgService kaSystemShopmsgService;
     ...
     * @return
     */
    @Log(title = "门店信息导入", businessType = BusinessType.IMPORT)
    @RequiresPermissions("kaSystemShopmsg:insert")
    @PostMapping("importExcel")
    @ResponseBody
    @Transactional(rollbackFor = Exception.class)
    @ApiOperation(value = "导入门店信息",notes = "根据导入的文件导入数据到门店信息表",response = JSONResult.class,httpMethod = "POST")
    public JSONResult importExcel(@ApiParam(name = "file",value = "导入门店信息文件file",required = true) @RequestParam("file") MultipartFile file, HttpServletResponse response){
        boolean boo = false;
        //设置表名
        String tablename = "ka_system_shopmsg";
        ...
        try {
            ...
            for (int i=1;i<sheetAt.getPhysicalNumberOfRows();i++){
                ...
                int msg;
                if (this.kaSystemShopmsgService.selectmsg(bean.beanMap.get("shop_code").toString())>=1){
                    msg = this.kaSystemShopmsgService.updatemsg(bean.beanMap);
                }else {
                    //调用添加数据的方法
                    msg = this.kaSystemShopmsgService.insertmsg(bean.beanMap);
                }
                if (msg>0){
                    boo = true;
                }
